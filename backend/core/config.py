...
class Settings(BaseSettings):
    ...
    # 主动式定时刷新 token：临近过期时后台重登换新（默认开启，token 寿命约 30 天）
    TOKEN_REFRESH_ENABLED: bool = _env_bool("TOKEN_REFRESH_ENABLED", True)
    TOKEN_REFRESH_CHECK_INTERVAL: int = int(os.getenv("TOKEN_REFRESH_CHECK_INTERVAL", 21600))  # 6h 扫描一次
    TOKEN_REFRESH_AHEAD_SECONDS: int = int(os.getenv("TOKEN_REFRESH_AHEAD_SECONDS", 259200))   # 提前 72h 刷
    TOKEN_REFRESH_STAGGER_MS: int = int(os.getenv("TOKEN_REFRESH_STAGGER_MS", 2000))           # 账号间错峰 2s
